...
def process_gcm_data(gdir, filesuffix='', prcp=None, temp=None,
    ...
    """
    temp = temp.sel(time=slice("1950", "2101"))
    prcp = prcp.sel(time=slice("1950", "2101"))
    ...
    fpath = gdir.get_filepath('climate_historical') #temp is in celsius
    with xr.open_dataset(fpath) as ds_ref:

        ds_ref = ds_ref.sel(time=slice(*year_range))#参考气候数据，ERA5-Land
        if apply_bias_correction:
            # compute monthly anomalies
            # of temp
            if scale_stddev: #是否标准化标准差
                # This is a bit more arithmetic
                ts_tmp_sel = temp.sel(time=slice(*year_range))#需要校正的数据
                ...
                if ((len(ts_tmp_sel) // 12) % 2) == 1:#需要偶数年
                    raise InvalidParamsError('We need an even number of years '
                                             'for this to work')
                ts_tmp_std = ts_tmp_sel.groupby('time.month').std(dim='time')#计算参考期间的月标准差，每个月一个值(目标数据)
                std_fac = ds_ref.temp.groupby('time.month').std(dim='time') / ts_tmp_std #参考数据月标准差/目标数据月标准差
                if sm != 1: #如果hydro_month_hemisphere不是1，则需要roll
                    # Just to avoid useless roll
                    std_fac = std_fac.roll(month=13-sm, roll_coords=True)
                std_fac = np.tile(std_fac.data, len(temp) // 12) #将标准化因子重复到目标数据的每个时间点
                # We need an even number of years for this to work
                win_size = len(ts_tmp_sel) + 1

                def roll_func(x, axis=None):
                    x = x[:, ::12] # 每12个月取一个值（每年同一月份）
                    n = len(x[0, :]) // 2 # 找到中间年份的索引
                    xm = np.nanmean(x, axis=axis)
                    return xm + (x[:, n] - xm) * std_fac

                temp = temp.rolling(time=win_size, center=True,
                                    min_periods=1).reduce(roll_func)# (1, )

            ts_tmp_sel = temp.sel(time=slice(*year_range))#参考时期的目标温度
            if len(ts_tmp_sel.time) != len(ds_ref.time):
                raise InvalidParamsError('The reference climate period and the '
                                         ...
                                         'not match.')
            ts_tmp_avg = ts_tmp_sel.groupby('time.month').mean(dim='time')#目标温度在参考期间的月均值
            ts_tmp = temp.groupby('time.month') - ts_tmp_avg#目标温度与参考期间月均值的差值
            # of precip -- scaled anomalies
            ts_pre_avg = prcp.sel(time=slice(*year_range))#参考期间的目标降水
            ts_pre_avg = ts_pre_avg.groupby('time.month').mean(dim='time')#目标降水在参考期间的月均值
            ts_pre_ano = prcp.groupby('time.month') - ts_pre_avg#目标降水与参考期间月均值的差值
            # scaled anomalies is the default. Standard anomalies above
            # are used later for where ts_pre_avg == 0
            ts_pre = prcp.groupby('time.month') / ts_pre_avg#目标降水与参考期间月均值的比值

            # for temp
            loc_tmp = ds_ref.temp.groupby('time.month').mean()#参考温度的月均值
            ts_tmp = ts_tmp.groupby('time.month') + loc_tmp#temp + 参考数据月均值 - 目标数据月均值，在参考期间两类数据能对上

            # for prcp
            loc_pre = ds_ref.prcp.groupby('time.month').mean()#参考降水的月均值
            # scaled anomalies
            ts_pre = ts_pre.groupby('time.month') * loc_pre#prcp * 参考数据月均值/目标数据在参考期间的月均值，在参考期间两类数据能对上
            # standard anomalies
            ts_pre_ano = ts_pre_ano.groupby('time.month') + loc_pre#prcp + 参考数据月均值 - 目标数据月均值
...
def process_cmip_data(gdir, filesuffix='', fpath_temp=None,
    ...
    **kwargs: any kwarg to be passed to ref:`process_gcm_data`
    ...
    with xr.open_dataset(fpath_temp, use_cftime=True) as tempds, \
        ...
        # Take the closest to the glacier
        # Should we consider GCM interpolation?
        temp = tempds.tas.sel(lat=glat, lon=glon, method='nearest')# a value
        precip = precipds.pr.sel(lat=glat, lon=glon, method='nearest') # a value

        temp = temp.sel(time=slice("1950", "2101"))
        precip = precip.sel(time=slice("1950", "2101"))
        temp = temp.load()
        precip = precip.load()
        ...
        ny, r = divmod(len(temp), 12)
        assert r == 0 # the complete years
        dimo = [cfg.DAYS_IN_MONTH[m - 1] for m in temp['time.month']] # days in each month
        precip = precip * dimo * (60 * 60 * 24) #mm/month
