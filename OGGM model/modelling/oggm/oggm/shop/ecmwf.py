...
BASENAMES = {
    ...
    'ERA5L-LATEST': {
        'inv': 'era5-land/monthly/latest/era5_land_invariant_flat_HMA.nc',
        'pre': 'era5-land/monthly/latest/era5_land_monthly_prcp_1950-2023_flat_HMA.nc',
        'tmp': 'era5-land/monthly/latest/era5_land_monthly_t2m_1950-2023_flat_HMA.nc'
    },
    'CERA': {
...
def get_ecmwf_file(dataset='ERA5', var=None):
    ...
    ----------
    dataset : str
        'ERA5', 'ERA5L', 'CERA', 'ERA5L-HMA', 'ERA5L-LATEST', 'ERA5dr'
    var : str
        'inv' for invariant
        'tmp' for temperature
...
def process_ecmwf_data(gdir, dataset=None, ensemble_member=0,
                       y0=None, y1=None, output_filesuffix=None, download=False, local_path_dict=None):
    ...
    lat = gdir.cenlat
    if download:  
        tmp_path = get_ecmwf_file(dataset, 'tmp')
        pre_path = get_ecmwf_file(dataset, 'pre')
        inv_path = get_ecmwf_file(dataset, 'inv')
    else:
        if local_path_dict is None:
            raise ValueError('local_path_dict is required if download is False')
        if 'tmp' not in local_path_dict or 'pre' not in local_path_dict or 'inv' not in local_path_dict:
            raise ValueError('local_path_dict must contain tmp, pre, and inv keys')
        tmp_path = local_path_dict['tmp']
        pre_path = local_path_dict['pre']
        inv_path = local_path_dict['inv']

    with xr.open_dataset(tmp_path) as ds: #temperature
        _check_ds_validity(ds)
        # set temporal subset for the ts data (hydro years)
        sm = cfg.PARAMS['hydro_month_' + gdir.hemisphere] #1
        ...
        ref_lon = ref_lon - 360 if ref_lon > 180 else ref_lon #（-180，180）
        ref_lat = float(ds['latitude'])
    with xr.open_dataset(pre_path) as ds:
        _check_ds_validity(ds)
        ds = ds.sel(time=slice('{}-{:02d}-01'.format(y0, sm),
                               '{}-{:02d}-01'.format(y1, em)))
        if dataset == 'CERA':
        ...
        try:
            ...
            c = (ds.longitude - lon)**2 + (ds.latitude - lat)**2
            ds = ds.isel(points=np.argmin(c.data))
        prcp = ds['tp'].data * 1000 * ds['time.daysinmonth'] #mm/month
    with xr.open_dataset(inv_path) as ds:
        _check_ds_validity(ds)
        ds = ds.isel(time=0)
        try:
            ds = ds.sel(longitude=lon, latitude=lat, method='nearest')
